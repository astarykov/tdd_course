...
public:
    ...
    double GetMaximumTemperature(IWeatherServer& server, const std::string& date)
    {
        ...
        for (int i = 0; i < int(serverResults.size()); i++) {
             maxTemp = maxTemp < serverResults[i].temperature ? serverResults[i].temperature : maxTemp ;
        }
...
TEST(weatherClient, getAvarageTemperatureForInProperDate) {
    MockWeatherServer server = MockWeatherServer();
    WeatherClient client = WeatherClient();
    ASSERT_EQ(client.GetAverageTemperature(server, "022018"), 0.0);
}
...
TEST(weatherClient, getMaximumTemperatureForProperDate) {
    MockWeatherServer server = MockWeatherServer();
    WeatherClient client = WeatherClient();
...
TEST(weatherClient, getMaximumWindSpeedForProperDate) {
    MockWeatherServer server = MockWeatherServer();
    WeatherClient client = WeatherClient();
