...
class CofeeMachine {
    ...
    void makeLatte(int capacity) {
        _ingredientSource->SetCupSize(capacity);
        _ingredientSource->AddWater(0, 90);
        _ingredientSource->AddCoffee(capacity/2);
        _ingredientSource->AddMilk(capacity/4);
        _ingredientSource->AddMilkFoam(capacity/4);
    }
...
TEST(Coffee, makeLatteOfSizeBig) {
    SourceOfIngredientsMock source;
    CofeeMachine machine(&source);

    EXPECT_CALL(source, SetCupSize(140)).Times(1);
    EXPECT_CALL(source, AddWater(0, 90)).Times(1);
    EXPECT_CALL(source, AddCoffee(140/2)).Times(1);
    EXPECT_CALL(source, AddMilk(140/4)).Times(1);
    EXPECT_CALL(source, AddMilkFoam(140/4)).Times(1);
    machine.makeLatte(140);
}

TEST(Coffee, makeMarochinoOfSizeSmall) {
    SourceOfIngredientsMock source;
    CofeeMachine machine(&source);

    EXPECT_CALL(source, SetCupSize(100)).Times(1);
    EXPECT_CALL(source, AddCoffee(100/4)).Times(1);
    EXPECT_CALL(source, AddMilk(100/4)).Times(1);
    EXPECT_CALL(source, AddChocolate(100/4)).Times(1);
    machine.makeMarochino(100);
}

TEST(Coffee, makeMarochinoOfSizeBig) {
    SourceOfIngredientsMock source;
    CofeeMachine machine(&source);

    EXPECT_CALL(source, SetCupSize(140)).Times(1);
    EXPECT_CALL(source, AddCoffee(140/4)).Times(1);
    EXPECT_CALL(source, AddMilk(140/4)).Times(1);
    EXPECT_CALL(source, AddChocolate(140/4)).Times(1);
    machine.makeMarochino(140);
}
